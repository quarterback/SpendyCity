...
const PREGENERATED_MEMOS = Object.fromEntries(
  Object.entries(memoFiles).map(([path, body]) => [slugFromPath(path), body.trimEnd()])
);
const cf = (item, units, unitCost, basis) => ({
  item,
  units,
  unitCost,
...
const PROPOSALS = {
  // ──────────────────────────────────────────────────────────────────────────
  "arts-tax": {
    voterIntentPlain: "You voted for two things in 2012. Pay K–5 art and music teachers in Portland-area schools. Fund grants so nonprofits can bring arts to kids who would not get them otherwise.",
    couldFund: [
      cf("K–5 art and music teachers, full-time, 5 years", 220, 84e3, "PPS Tier-3 teacher loaded salary, 2025"),
      cf("Student arts intensive scholarships", 14e3, 1650, "RACC Arts for All published rate"),
      cf("K–5 instruments and maintenance", 12e3, 460, "PPS music program procurement"),
      cf("Bilingual arts education across all 5 districts, 5 years", 1, 12e6, "$2.4M per year, 5-district pilot scale")
    ],
    blockers: [
      {
        name: "The 5% overhead cap was rewritten instead of enforced",
        mechanism: 'The 2012 ballot capped collection costs at 5% of revenue. When costs went over, Council redefined what counts as "collection cost" instead of cutting it.',
        controlledBy: "Portland City Council, four votes",
        defense: "Collection costs are necessarily higher for a flat tax with a broad filer base. The cap as written was unworkable.",
        rebuttal: "King County's per-capita arts tax hits the same compliance at under 4% overhead. The cost is a choice, not a constraint."
      },
      {
        name: "No floor on the teacher line",
        mechanism: "The ordinance does not require any minimum share of revenue to go to K–5 teacher salaries. Teacher dollars compete with grant dollars and overhead each year.",
        controlledBy: "Portland City Council, four votes",
        defense: "Annual flexibility lets the fund respond to changing arts ecosystem needs.",
        rebuttal: "The ballot text named two uses. A floor on the larger one would honor what voters passed."
      }
    ],
    ifUnblocked: "Cap overhead at 5% and put a floor on the teacher line, and a music or art teacher returns to every K–5 school in PPS, Centennial, David Douglas, Parkrose, and Reynolds within two years.",
    blockerNews: "The City Auditor's 2024 follow-up found compliance still under 75% and overhead unchanged. Council has not taken action on either finding."
  },
  // ──────────────────────────────────────────────────────────────────────────
  pcef: {
    voterIntentPlain: "You voted in 2018 to make big retailers pay 1% of their Portland sales into a fund for clean-energy projects, green jobs training, and weatherization, designed to benefit communities of color and low-income residents first.",
    couldFund: [
      cf("Home weatherization retrofits for low-income households", 12e3, 32e3, "BPS published retrofit cost, 2024"),
      cf("BIPOC clean-energy career training (NABCEP Tier-1) over 5 years", 800, 48e3, "NABCEP tuition + stipend + placement"),
      cf("Community microgrids in East Portland neighborhoods", 6, 14e6, "BPA / EnergyTrust microgrid pilot scale"),
      cf("Heat-pump replacements for low-income homes", 4200, 38e3, "EnergyTrust HVAC program installed cost"),
      cf("Cool roofs across every K–5 school in the 5 east-side districts", 24e5, 4.2, "Cool Roof Coalition $4.20/sqft, 2.4M sqft scoped")
    ],
    blockers: [
      {
        name: 'Council can rewrite "eligible uses" with a simple-majority vote',
        mechanism: 'PCC 7.07 lets Council redefine what counts as a "clean energy project" through a normal Council vote. There is no return to voters.',
        controlledBy: "Portland City Council, four votes",
        defense: "Flexibility is needed to respond to changing climate priorities and align with city budget realities.",
        rebuttal: "The ballot text named retrofits, training, and BIPOC workforce. LED lighting at a private arena is general municipal capital, not a climate priority."
      },
      {
        name: "No minimum-deployment ratio in the ordinance",
        mechanism: "Nothing requires PCEF to spend a fixed share of inflow each year on the original program lines. Surplus accumulates indefinitely.",
        controlledBy: "Portland City Council, four votes",
        defense: "Multi-year planning produces better projects than spend-it-or-lose-it cycles.",
        rebuttal: "Six years of collections have not produced an actual five-year deployment plan. The argument has not produced the plan."
      },
      {
        name: 'Grants reported as "awarded," not as delivered',
        mechanism: "PCEF reports grants on the date Council approves them, not on the date the work happens. Awarded does not equal weatherized, trained, or installed.",
        controlledBy: "PCEF Bureau reporting practice; Council can require quarterly reconciliation",
        defense: "Awards represent commitment. Reporting on completion would be misleading because projects span years.",
        rebuttal: "Federal grants do quarterly completion reporting routinely. There is no technical reason it can't be done here."
      }
    ],
    ifUnblocked: "Restore the original mandate and PCEF puts 800 Portlanders into clean-energy careers and weatherizes roughly one in eight low-income homes in the city within five years.",
    blockerNews: `In April 2026, PCEF dollars were proposed to fund LED lighting, HVAC upgrades, and concourse renovations at the privately-operated Moda Center under "eligible green infrastructure" language. The arena's operating cost goes down. The original BIPOC workforce mandate goes unfunded.`
  },
  // ──────────────────────────────────────────────────────────────────────────
  "housing-investment": {
    voterIntentPlain: "A 2006 city policy says 30 cents of every urban-renewal dollar must go to building affordable housing inside the same neighborhood that generated it. The set-aside is supposed to be honored district by district.",
    couldFund: [
      cf("Deeply-affordable units at 60% AMI", 320, 375e3, "Bond program blended unit cost, 2024"),
      cf("Land trust acquisitions to lock long-term affordability", 1400, 84e3, "PHB land trust acquisition average"),
      cf("Home-repair grants for low-income owners (anti-displacement)", 3e3, 4e4, "PHB home-repair grant ceiling")
    ],
    blockers: [
      {
        name: 'The definition of "affordable" was widened to 120% AMI',
        mechanism: "Workforce-grade and amenity-rich units now count toward the set-aside. The headline 30% holds. The depth of affordability has dropped.",
        controlledBy: "Portland Housing Bureau policy; Council ratification",
        defense: "Workforce housing serves essential workers near opportunity and is a recognized HUD category.",
        rebuttal: "120% AMI in the PPS catchment is over $135,000 a year. Voters approved affordable. Anything else is policy invention."
      },
      {
        name: "Cross-district swaps defer obligations to future years",
        mechanism: "Set-aside obligations from one TIF district can be moved to another with a different schedule. Reported compliance stays high while delivery slips.",
        controlledBy: "Council resolution, by simple majority",
        defense: "Cross-district transfers let dollars follow market readiness.",
        rebuttal: "Prosper Portland's own 2021 memo names a multi-year backlog. Transfers compounded the deferral."
      }
    ],
    ifUnblocked: "Pull the definition back to 80% AMI and require district-level delivery, and the existing backlog clears as roughly 1,200 deeply-affordable units within four years.",
    blockerNews: "The 2018 district-swap resolution remains in force. The original obligations have not been reattached to their original districts."
  },
  // ──────────────────────────────────────────────────────────────────────────
  "rental-services": {
    voterIntentPlain: "Landlords pay an annual fee for every unit they rent in Portland. The money is supposed to go to tenant counseling, the Rental Housing Hotline, fair-housing investigations, and dispute resolution. It pays for help when something goes wrong with your landlord.",
    couldFund: [
      cf("Full-time tenant counselors for 4 years", 60, 65e3, "City Tier-3 grant scale loaded, 2025"),
      cf("Doubling of Rental Housing Hotline staffing for 5 years", 1, 9e6, "$1.8M/year, 5 years, current staffing baseline"),
      cf("Fair-housing investigations", 12e3, 1300, "Fair Housing Council average per-case cost"),
      cf("Rapid-rehousing payments for displacement-risk households", 2400, 5e3, "JOHS rapid-rehousing per-household average")
    ],
    blockers: [
      {
        name: "Year-end balance can be swept to housing-bureau overhead",
        mechanism: `The adopted budget can reallocate any unspent rental-services balance to the Housing Bureau's general overhead under "operational support."`,
        controlledBy: "Portland City Council, in adopted budget",
        defense: "Operational support is necessary for service-delivery infrastructure.",
        rebuttal: "Bureau overhead is funded by general appropriation. Sweeping a fee paid by landlords for tenant services bypasses what the program was created for."
      },
      {
        name: "No floor on tenant-services spending",
        mechanism: "There is no rule requiring a minimum share of inflow each year to actually reach the program lines the fee is collected for.",
        controlledBy: "Council ordinance amendment",
        defense: "Conservative spending ensures a multi-year reserve for crisis response.",
        rebuttal: "Hotline wait times exceed the program's own service standards while the reserve grows. The crisis is now."
      }
    ],
    ifUnblocked: "Stop the sweeps and put a floor on the counselor line, and the current hotline backlog clears within a year. Tenants in trouble get a callback the same week.",
    blockerNews: "The FY 2022 budget reallocated the year-end balance to bureau overhead. The audit recommendation to set a tenant-services floor has not been implemented."
  },
  // ──────────────────────────────────────────────────────────────────────────
  "affordable-housing-dev": {
    voterIntentPlain: "In 2018, voters across three counties approved a $652.8M bond to build or preserve 3,900 affordable homes for working families, seniors, veterans, and people with disabilities.",
    couldFund: [
      cf("Newly-built affordable units at corrected unit cost", 1060, 41e4, "Metro bond program 2024 unit cost"),
      cf("Preservation of existing affordable units", 1100, 165e3, "Metro preservation program per-unit"),
      cf("Site acquisition for the next round of units", 3790, 115e3, "Metro land acquisition program average")
    ],
    blockers: [
      {
        name: "Cost growth was absorbed by lowering the unit count",
        mechanism: "When per-unit construction costs outran the bond assumption, jurisdictions kept the dollar total fixed and reduced the home count. Voters approved a unit count.",
        controlledBy: "Metro Council and partner-jurisdiction bureaus",
        defense: "Construction inflation is outside Metro's control.",
        rebuttal: "Yet Metro chose to absorb cost growth by lowering targets rather than supplementing the bond. Voters approved 3,900 homes, not $652.8M."
      },
      {
        name: '"Preserved" units count toward the 3,900-home headline',
        mechanism: "The mix between newly built and preserved units has shifted toward preservation while the headline number stays the same. The two are not equivalent.",
        controlledBy: "Metro reporting practice; jurisdiction project selection",
        defense: "Preservation prevents displacement and is counted by HUD as a housing strategy.",
        rebuttal: "The ballot title said build or preserve. The bureau choices have shifted the mix away from build without returning to voters."
      }
    ],
    ifUnblocked: "A supplemental bond and a binding split between built and preserved would land 3,900 homes within the original schedule plus two years.",
    blockerNews: "A 2024 Metro action extended the delivery window past the original sunset. Counts have not been re-baselined to the original built-versus-preserved split."
  },
  // ──────────────────────────────────────────────────────────────────────────
  "preschool-for-all": {
    voterIntentPlain: "In 2020, Multnomah County voters approved a tax on high incomes to give every 3- and 4-year-old in the county a tuition-free spot at a high-quality preschool, paid teachers a living wage, and built up provider capacity.",
    couldFund: [
      cf("Tuition-free preschool seats over 5 years (full ramp)", 11200, 14e3, "County provider rate, modeled blended"),
      cf("Workforce wage uplift to a $28/hr floor for early educators", 1400, 38e3, "$28/hr × 1,950 hr × 4 yr — wage delta only"),
      cf("New provider classrooms", 380, 145e3, "County classroom buildout average")
    ],
    blockers: [
      {
        name: "No published binding ramp schedule",
        mechanism: "The county controls the enrollment ramp internally. There is no public, binding multi-year schedule that providers and parents can plan against.",
        controlledBy: "Multnomah County Preschool & Early Learning Division",
        defense: "Provider capacity must build before enrollment can scale; rushing risks quality.",
        rebuttal: "Other counties have ramped at twice this pace using the same workforce dollars. The slow ramp is a choice, not a constraint."
      },
      {
        name: 'Surplus is framed as "future capacity reserve" with no spend trigger',
        mechanism: "Year-end balances are reported as reserves rather than as undeployed capacity. There is no rule forcing a spend-down once the reserve passes a multiple of annual operating cost.",
        controlledBy: "County Board of Commissioners",
        defense: "Reserves protect against tax-rate volatility.",
        rebuttal: "A reserve at one year of operations would be defensible. The current modeled reserve is over three."
      }
    ],
    ifUnblocked: "Publish a binding 5-year ramp with quarterly enrollment reports, and 11,200 children get seats while 1,400 educators move to the wage floor.",
    blockerNews: "In 2024 the County Board deferred a vote on adjusting the tax rate while enrollment continued to lag. The reserve framing was preserved."
  },
  // ──────────────────────────────────────────────────────────────────────────
  "supportive-housing": {
    voterIntentPlain: "In 2020, voters in Multnomah, Washington, and Clackamas counties approved taxes dedicated to ending chronic homelessness through long-term rent help, behavioral-health services, outreach, and permanent supportive housing.",
    couldFund: [
      cf("Long-term rent-assistance vouchers for 3 years", 12e3, 50400, "$1,400/mo × 36 months"),
      cf("Newly-built supportive-housing units (capital + 5-yr operating)", 1300, 48e4, "JOHS supportive housing unit cost"),
      cf("Assertive Community Treatment teams (96 teams, 1 year)", 96, 21e5, "ACT model annual cost per team"),
      cf("Outreach workers loaded for 4 years", 240, 384e3, "$96k loaded × 4 yr")
    ],
    blockers: [
      {
        name: "Counties under-execute against contracted service capacity",
        mechanism: "Year-by-year contracts repel providers; multi-year contracts (which counties have authority to sign) would attract them. The contract structure is the binding constraint, not dollars.",
        controlledBy: "County procurement and contracting offices",
        defense: "Provider capacity is the binding constraint, not dollars.",
        rebuttal: "Multi-year provider funding agreements would attract capacity. Year-by-year contracts repel it. The constraint is procurement, not the market."
      },
      {
        name: '"Long-term program reserves" framing protects unspent money from disbursement',
        mechanism: "Carryover balances are reported in financial statements as long-term reserves rather than as obligations behind on delivery. The framing slows the disbursement clock.",
        controlledBy: "County financial reporting practice",
        defense: "Reserves protect against tax revenue volatility.",
        rebuttal: "A volatility reserve at one year of operations is standard. The current carryover is multiples of that."
      },
      {
        name: 'New "supportive housing services" categories added without a return to voters',
        mechanism: "The ballot text named four uses. New service categories are being added under the same fund through county-level intergovernmental agreements.",
        controlledBy: "Tri-county intergovernmental agreement process",
        defense: "Scope adjustments respond to evolving service models in the field.",
        rebuttal: "Scope additions belong in front of voters, or out of this fund. Otherwise the dedicated tax becomes a general one."
      }
    ],
    ifUnblocked: "Sign multi-year provider contracts and reattach the four ballot uses as the only eligible uses, and 12,000 households get rent help while 1,800 supportive-housing beds come online within four years.",
    blockerNews: 'The 2025 Metro Auditor follow-up flagged new categories of "supportive housing services" being added without explicit voter authorization. Counties continue to add categories.'
  }
};
const MM = 1e6;
...
function buildReserveSeries(cash) {
...
const FUNDS = [
  {
    ...
    voterIntent: "Restore K–5 arts and music teachers in Portland-area public schools and fund arts access grants for nonprofits, paid for by a $35 per-adult tax on income above $1,000.",
    oneLineStatus: 'Collections lag, administrative overhead is structurally high, and the Council periodically authorizes carve-outs to "support arts ecosystem" outside the schools-and-grants frame.',
    ...
    scandal: 'A flat tax billed as a teacher-funding measure spends roughly one in eight dollars on the cost of collecting itself, and Council periodically expands what counts as "arts."',
  ...
  },
  {
    ...
    restrictionClass: "voter-restricted",
    voterIntent: "A 1% surcharge on retail sales of large corporations operating in Portland, dedicated to clean-energy projects, workforce training, and green infrastructure benefitting communities of color and low-income residents.",
    oneLineStatus: "Collections vastly outran original projections; the Council subsequently authorized broader uses including general transportation, parks, and sewer maintenance — items not contemplated in the ballot text.",
    ...
    scandal: 'A surcharge passed for clean energy and BIPOC workforce now backfills street paving and parks budgets after Council expanded "eligible uses" by simple majority.',
    cashSeries: pcefCash,
  ...
  },
  {
    ...
    voterIntent: "A policy commitment that 30% of urban renewal tax-increment financing (TIF) revenue is set aside for affordable housing development inside each district.",
    oneLineStatus: 'Set-aside percentages have been honored in headline reporting but partially offset by reclassifying eligible projects, swapping districts, and converting "obligations" into deferred allocations.',
    ...
    scandal: 'A 30% set-aside policy has been re-engineered through reclassification: projects counted as "affordable" include workforce-grade and market-rate-with-amenities developments.',
  ...
  },
  {
    ...
    voterIntent: "Annual per-unit landlord registration fee dedicated to tenant counseling, the Rental Housing Hotline, fair-housing investigations, and dispute resolution.",
    oneLineStatus: "Fee revenue is routinely under-deployed against the program lines it was created for; balances have been periodically swept toward general housing administration.",
    ...
    scandal: "A landlord-paid fund for tenant services consistently spends less than half its inflow on tenant services, with the remainder absorbed into housing-bureau overhead.",
  ...
  },
  {
    ...
    restrictionClass: "voter-restricted",
    voterIntent: "A regional general-obligation bond authorizing $652.8M to build or preserve 3,900 affordable homes for working families, seniors, veterans, and people with disabilities.",
    oneLineStatus: 'Unit counts have been adjusted downward relative to original targets; per-unit costs have grown well above the projection range; "preserved" vs "built" classification has shifted to maintain headline numbers.',
    ...
    scandal: 'A 3,900-home bond is on track to deliver fewer homes per dollar than promised; jurisdictions have re-classified "preservation" of existing units to keep count metrics intact.',
  ...
  },
  {
    ...
    restrictionClass: "voter-restricted",
    voterIntent: "A graduated personal income tax dedicated to providing tuition-free, high-quality preschool to all 3- and 4-year-olds in Multnomah County, with workforce wage standards.",
    oneLineStatus: "Collections sharply outran ramp-up capacity, producing a multi-hundred-million accumulated balance even as enrollment lagged the implementation plan.",
    ...
    scandal: "A tuition-free preschool tax has built a half-billion-dollar surplus while serving a fraction of the eligible 3- and 4-year-olds the measure was designed to cover.",
  ...
  },
  {
    ...
    restrictionClass: "voter-restricted",
    voterIntent: "Tri-county taxes dedicated to long-term rent assistance, behavioral health services, outreach, and supportive housing to end chronic homelessness in the region.",
    oneLineStatus: 'Counties have collected far more than they could disburse against contracted capacity; carryover balances are being reframed as "long-term reserves" without clear voter mandate.',
    ...
    scandal: "A homelessness measure has accumulated a multi-hundred-million dollar reserve while service contracts go unfilled and rent assistance waitlists grow.",
...
const FUND_BY_SLUG = Object.fromEntries(FUNDS.map((f) => [f.slug, f]));
const TOTAL_MODELED_BALANCE = FUNDS.reduce((s, f) => s + f.modeledBalance, 0);
const TOTAL_CUMULATIVE_COLLECTED = FUNDS.reduce((s, f) => s + f.cumulativeCollected, 0);
const TOTAL_RESTRICTED = FUNDS.reduce(
  (s, f) => s + f.modeledBalance * f.modeledRestrictedShare,
  0
);
const TOTAL_MOVABLE = FUNDS.reduce(
  (s, f) => s + f.modeledBalance * f.modeledMovableShare,
  0
);
export {
  FUND_BY_SLUG as F,
  TOTAL_MODELED_BALANCE as T,
  FUNDS as a,
  TOTAL_CUMULATIVE_COLLECTED as b,
  TOTAL_RESTRICTED as c,
  TOTAL_MOVABLE as d
};
